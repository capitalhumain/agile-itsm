...
public class RenderMenu {
    ...
    public static String render(final Collection colMenus, final String contextName) throws Exception {
        final ItemController itemController = new ItemController();
        final String strCab = "var menu = new Array();\n" + "var defOver = '" + Constantes.getValue("MENU_defOver") + "', defBack = '"
                + Constantes.getValue("MENU_defBack") + "', defOver2 = '" + Constantes.getValue("MENU_defOver2") + "', defBack2 = '"
                + Constantes.getValue("MENU_defBack2") + "';\n" + "var defLength = 20;\nvar posicao = 00;\n" + "menu[0] = new Array();\n"
                + "menu[0][0] = new Menu(true, '', 0, 90, " + Constantes.getValue("MENU_tamMenu") + ", defOver, defBack, 'menuBorda', 'menu');\n";

        String strBuffer = "";
        String strBufferSubMenus = "";
        final StringBuilder sMenuSubNiveis = new StringBuilder("");
    ...
    private static String renderRoot(final MenuItem m, final int iRoot, final int iPos) {
        String str = "";
        str += "menu[0][" + iRoot + "] = new Item('" + m.getDescription() + " <img src=\"../../imagens/seta_link1.gif\" border=0>\', '#', '', defLength, 0, "
                + iRoot + ", 1, '<img src=\"../../imagens/seta_branca.gif\" border=0>&nbsp;&nbsp;', 0," + iPos + ");";
        str += "\n";

        return str;
    }

    private static String renderItens(final MenuItem m, final int iItem, final ItemController itemController, final StringBuilder sMenuSubNiveis,
            final boolean subMenu, final String contextName) throws Exception {
        if (m == null) {
            return "";
        }
        ...
        if (m.getMenuItens() != null) {
            ...
            while (it.hasNext()) {
                ...
                final String plusHeight = mTemp.getDescription().length() > 21 ? "" : "";
                str += "menu[" + iItem + "][" + iInterno + "] = new Item('" + mTemp.getDescription() + " " + strImgSubItem + "', '"
                        + renderPath(mTemp.getPath(), contextName) + "', '', defLength" + plusHeight + ", 0, " + iRootAux + ", 1, '', 1," + pos + ");";
                str += "\n";
                iInterno++;
                pos += 20;
    ...
    public static String renderPath(final String path, final String contextName) throws Exception {
        if (path == null) {
            return "#";
        }
        ...
        final String ctxNew = contextName.replaceAll("\\#", "\\&");
