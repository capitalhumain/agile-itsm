...
public class PostDAO extends CrudDaoDefaultImpl {
    ...
    public Collection listNotNull() throws Exception {
        ...
        if (CITCorporeUtil.SGBD_PRINCIPAL.equalsIgnoreCase("ORACLE")) {
            sql = "SELECT idPost, titulo, descricao, conteudo, imagem, dataInicio FROM " + this.getTableName()
                    + " WHERE dataFim IS NULL AND ROWNUM <= 3 ORDER BY idPost DESC";
        } else if (CITCorporeUtil.SGBD_PRINCIPAL.equalsIgnoreCase(SQLConfig.SQLSERVER)) {
            sql = " SELECT TOP (3) idPost, titulo, descricao, conteudo, imagem, dataInicio FROM " + this.getTableName()
                    + " WHERE dataFim IS NULL  ORDER BY idPost DESC";
        } else {
            sql = "SELECT idPost, titulo, descricao, conteudo, imagem, dataInicio FROM " + this.getTableName()
                    + " WHERE dataFim IS NULL ORDER BY idPost DESC LIMIT 3";
        }
    ...
    public Collection findByConditionByCategoria(final Integer idCategoriaPost) throws Exception {
        final List list1 = new ArrayList<>();
        final List list2 = new ArrayList<>();
