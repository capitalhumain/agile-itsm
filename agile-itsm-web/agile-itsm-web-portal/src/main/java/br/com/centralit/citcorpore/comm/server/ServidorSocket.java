    ...
    public void run() {
        ...
        try {
            while (true) {
                ...
                final String[] array = ipCliente.split(":");
                ipCliente = array[0];
                ipParaBusca = ipCliente.replace(".", "-");

                final InventarioXMLService inventarioXMLService = (InventarioXMLService) ServiceLocator.getInstance().getService(InventarioXMLService.class,
                        null);
                /*
                 * ParametroCorporeService parametroService = (ParametroCorporeService) ServiceLocator.getInstance().getService(ParametroCorporeService.class,
                 * null);
                 * ParametroCorporeDTO paramentroDTO = null;
                 * List<ParametroCorporeDTO> listDiasInventario = parametroService.pesquisarParamentro(Enumerados.ParametroSistema.DiasInventario.id(),
                 * Enumerados.ParametroSistema.DiasInventario.campo());
                ...
                final Date dataInventario = UtilDatas.getSqlDate(new java.util.Date());

                final boolean inventarioAtualizado = inventarioXMLService.inventarioAtualizado(ipParaBusca, dataInventario);
