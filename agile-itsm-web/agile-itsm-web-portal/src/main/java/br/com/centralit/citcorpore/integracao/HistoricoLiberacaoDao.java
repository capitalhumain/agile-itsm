...
public class HistoricoLiberacaoDao extends CrudDaoDefaultImpl {
    ...
    public HistoricoLiberacaoDTO maxIdHistorico(final RequisicaoLiberacaoDTO requisicaoLiberacaoDTO) throws PersistenceException {
        final List parametro = new ArrayList<>();
        final List listRetorno = new ArrayList<>();
        final StringBuilder sql = new StringBuilder();

        sql.append("SELECT MAX(idhistoricoliberacao) AS idhistoricoliberacao FROM " + this.getTableName() + " WHERE idliberacao = ?");
        ...
        final List result = this.listConvertion(HistoricoLiberacaoDTO.class, resultado, listRetorno);
    ...
    public List<HistoricoLiberacaoDTO> listHistoricoLiberacaoByIdRequisicaoLiberacao(final Integer idRequisicaoLiberacao) throws PersistenceException {
        final List listRetorno = new ArrayList<>();
        final List parametro = new ArrayList<>();
        final String sql = "SELECT HIST.IDHISTORICOLIBERACAO, HIST.IDLIBERACAO, HIST.IDSOLICITANTE, "
                + "HIST.VERSAO, HIST.STATUS, HIST.IDEXECUTORMODIFICACAO, HIST.DATAHORAMODIFICACAO, "
                + "HIST.HISTORICOVERSAO, EMP.NOME ,USU.NOME, HIST.BASELINE " + "FROM HISTORICOLIBERACAO HIST "
        ...
        return this.listConvertion(HistoricoLiberacaoDTO.class, resultado, listRetorno);
    }
