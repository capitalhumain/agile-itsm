...
public abstract class RegraProgramacaoAtividade implements Serializable {
    ...
    public static RegraProgramacaoAtividade getRegraFromDto(final ProgramacaoAtividadeDTO programacaoAtividadeDto) throws Exception {
        return (RegraProgramacaoAtividade) Class.forName(RegraProgramacaoAtividade.class.getName() + "_" + programacaoAtividadeDto.getTipoAgendamento())
                .newInstance();
    }

    public static java.util.Date getDataProximaExecucao(final ProgramacaoAtividadeDTO programacaoAtividadeDto, final java.util.Date dataRef) throws Exception {
        return getRegraFromDto(programacaoAtividadeDto).calculaProximaExecucao(programacaoAtividadeDto, dataRef);
    }

    public static Integer[] getDiasUteis(final int mes, final int ano) throws Exception {
        final FeriadoDao feriadoDao = new FeriadoDao();
        final Integer[] diasUteis = new Integer[100];
        ...
        while (UtilDatas.getMonth(dataRef) == mes) {
            final int diaSemana = Util.getDiaSemana(dataRef);
    ...
    public static HashMap getSemanas(final int mes, final int ano) throws Exception {
        final Integer[] contSemanas = new Integer[] {0, 0, 0, 0, 0, 0, 0, 0};
        final HashMap<Integer, Integer[]> mapSemanas = new HashMap<>();
        ...
        while (UtilDatas.getMonth(dataRef) == mes) {
            ...
            final int semana = contSemanas[diaSemana].intValue();
            final Integer[] dias = mapSemanas.get(diaSemana);
    ...
    public static void validaProgramacao(final ProgramacaoAtividadeDTO programacaoAtividadeDto) throws Exception {
        ...
        final RegraProgramacaoAtividade regraImpl = getRegraFromDto(programacaoAtividadeDto);
        regraImpl.valida(programacaoAtividadeDto);
        setDescricao(programacaoAtividadeDto);
    }

    public static void setDescricao(final ProgramacaoAtividadeDTO programacaoAtividadeDto) throws Exception {
        final RegraProgramacaoAtividade regraImpl = getRegraFromDto(programacaoAtividadeDto);
    ...
    public String getRepeticaoDescr(final ProgramacaoAtividadeDTO programacaoAtividadeDto) throws Exception {
        String descricao = "";
        if (programacaoAtividadeDto.getRepeticao().equals("S")) {
    ...
    public abstract java.util.Date calculaProximaExecucao(final ProgramacaoAtividadeDTO programacaoAtividadeDto, final java.util.Date dataRef) throws Exception;

    public abstract String getDetalhamento(final ProgramacaoAtividadeDTO programacaoAtividadeDto) throws Exception;

    public abstract void valida(final ProgramacaoAtividadeDTO programacaoAtividadeDto) throws Exception;
