...
public class RegraProgramacaoAtividade_S extends RegraProgramacaoAtividade {
    ...
    public java.util.Date calculaProximaExecucao(final ProgramacaoAtividadeDTO programacaoAtividadeDto, final java.util.Date dataRef) throws Exception {
        ...
        if (dataExecucao.compareTo(dataRef) <= 0) {
            final String[] dias = new String[] {"", programacaoAtividadeDto.getDom(), programacaoAtividadeDto.getSeg(), programacaoAtividadeDto.getTer(),
            ...
            while (dataExecucao.compareTo(dataRef) < 0) {
                final int diaSemana = Util.getDiaSemana(dataExecucao);
    ...
    public void valida(final ProgramacaoAtividadeDTO programacaoAtividadeDto) throws Exception {
        ...
        if (programacaoAtividadeDto.getSeg().equals("N") && programacaoAtividadeDto.getTer().equals("N") && programacaoAtividadeDto.getQua().equals("N")
                && programacaoAtividadeDto.getQui().equals("N") && programacaoAtividadeDto.getSex().equals("N") && programacaoAtividadeDto.getSab().equals("N")
                && programacaoAtividadeDto.getDom().equals("N")) {
            throw new LogicException("Não foi informado nenhum dia da semana");
        }
        final String[] dias = new String[] {"", programacaoAtividadeDto.getDom(), programacaoAtividadeDto.getSeg(), programacaoAtividadeDto.getTer(),
                programacaoAtividadeDto.getQua(), programacaoAtividadeDto.getQui(), programacaoAtividadeDto.getSex(), programacaoAtividadeDto.getSab()};
        final int diaSemana = Util.getDiaSemana(programacaoAtividadeDto.getDataInicio());
