		...
		try {
			...
			if (colRequisicoes != null) {
				for (RequisicaoProdutoDTO requisicaoDto : colRequisicoes) {
					...
					col.add(new Object[] { requisicaoDto.getNomeUnidadeSolicitante(), requisicaoDto.getIdSolicitacaoServico(), requisicaoDto.getDataHoraSolicitacaoStr(), requisicaoDto.getContrato(),
							requisicaoDto.getCentroCusto(), requisicaoDto.getProjeto(), requisicaoDto.getServico(), requisicaoDto.getDescrSituacao(), valor });
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
