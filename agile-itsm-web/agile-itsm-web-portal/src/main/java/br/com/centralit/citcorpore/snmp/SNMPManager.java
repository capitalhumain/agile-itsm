...
public class SNMPManager {
     ...
     * @param add
     */
    public SNMPManager(final String add) {
        address = add;
    }

    public SNMPManager(final String add, final String comm) {
        address = add;
        community = comm;
    }

    public static String getSNMPXML(final String ipHostName, final String port, final String communityParm) throws Exception {
    ...
    public static String getSNMPSystem(final String ipHostName, final String port, final String communityParm) throws Exception {
        final SNMPManager client = new SNMPManager("udp:" + ipHostName + "/" + port, communityParm);
        client.start();
        String sysDescr = "";
        String sysUpTime = "";
        String sysName = "";
    ...
    public static String getSNMPStorages(final String ipHostName, final String port, final String communityParm) throws Exception {
        final SNMPManager client = new SNMPManager("udp:" + ipHostName + "/" + port, communityParm);
        client.start();
        String ret = "";
        for (int i = 1; i < 100; i++) {
            ...
            if (sysDiskName != null && !sysDiskName.trim().equalsIgnoreCase("") && !sysDiskName.trim().equalsIgnoreCase("Null")) {
                ...
                // --
                if (sysDiskName != null && !sysDiskName.trim().equalsIgnoreCase("") && !sysDiskName.trim().equalsIgnoreCase("noSuchInstance")
                        && !sysDiskName.trim().equalsIgnoreCase("noSuchObject")) {
                    ret = ret + "<STORAGES>\n";
    ...
    public static String getSNMPSoftwares(final String ipHostName, final String port, final String communityParm) throws Exception {
        final SNMPManager client = new SNMPManager("udp:" + ipHostName + "/" + port, communityParm);
        client.start();
        String ret = "";
        for (int i = 1; i < 500; i++) {
            ...
            if (sofName != null && !sofName.trim().equalsIgnoreCase("") && !sofName.trim().equalsIgnoreCase("Null")) {
                ...
                if (sofName != null && !sofName.trim().equalsIgnoreCase("") && !sofName.trim().equalsIgnoreCase("noSuchInstance")
                        && !sofName.trim().equalsIgnoreCase("noSuchObject")) {
                    ret = ret + "<SOFTWARES>\n";
    ...
    public static String getSNMPNetworks(final String ipHostName, final String port, final String communityParm) throws Exception {
        final SNMPManager client = new SNMPManager("udp:" + ipHostName + "/" + port, communityParm);
        client.start();
        String ret = "";
        for (int i = 1; i < 500; i++) {
            ...
            if (descrNet != null && !descrNet.trim().equalsIgnoreCase("") && !descrNet.trim().equalsIgnoreCase("Null")) {
                ...
                // installDate = installDate.replaceAll(":", "");
                // OctetString octetString = new OctetString(installDate);
                if (descrNet != null && !descrNet.trim().equalsIgnoreCase("") && !descrNet.trim().equalsIgnoreCase("noSuchInstance")
                        && !descrNet.trim().equalsIgnoreCase("noSuchObject")) {
    ...
    public static String fromHex(final String s) throws UnsupportedEncodingException {
        try {
    ...
    public static String convertHexToIP(final String hex) {
        String ip = "";

        for (int j = 0; j < hex.length(); j += 2) {
            final String sub = hex.substring(j, j + 2);
            final int num = Integer.parseInt(sub, 16);
            ip += num + ".";
        }
    ...
    /**
     * Start the Snmp session. If you forget the listen() method you will not get any answers because the communication is asynchronous and the listen() method
     * listens for answers.
     *
     * @throws IOException
     */
     ...
     * @throws IOException
     */
    public ResponseEvent get(final OID oids[]) throws IOException {
