...
public class RegraEscalonamentoDAO extends CrudDaoDefaultImpl {
     ...
     * 3- Mudança
     */
    public Collection findByTipoSolicitacao(final int idTipoGerenciamento) throws PersistenceException {
    ...
    public Collection<RegraEscalonamentoDTO> findRegraBySolicitacao(final SolicitacaoServicoDTO solicitacaoServicoDTO, final Integer idTipoGerenciamento)
        ...
        if (list != null && !list.isEmpty()) {
            ...
            final Collection<RegraEscalonamentoDTO> lista = this.listConvertion(this.getBean(), list, listRetorno);
            final Collection<RegraEscalonamentoDTO> novaLista = new ArrayList<RegraEscalonamentoDTO>(lista);
            for (final RegraEscalonamentoDTO regraEscalonamentoDTO : lista) {
                if (!temServico && regraEscalonamentoDTO.getIdServico() != null || temServico && regraEscalonamentoDTO.getIdServico() != null
                    ...
                    novaLista.remove(regraEscalonamentoDTO);
                } else if (!temSolicitante && regraEscalonamentoDTO.getIdSolicitante() != null || temSolicitante
                        && regraEscalonamentoDTO.getIdSolicitante() != null
                    ...
                    novaLista.remove(regraEscalonamentoDTO);
                } else if (!temDemandaServico && regraEscalonamentoDTO.getIdTipoDemandaServico() != null || temDemandaServico
                        && regraEscalonamentoDTO.getIdTipoDemandaServico() != null
                        && solicitacaoServicoDTO.getIdTipoDemandaServico().intValue() != regraEscalonamentoDTO.getIdTipoDemandaServico().intValue()) {
                    novaLista.remove(regraEscalonamentoDTO);
                } else if (!temImpacto && regraEscalonamentoDTO.getImpacto() != null && !regraEscalonamentoDTO.getImpacto().isEmpty() || temImpacto
                        && regraEscalonamentoDTO.getImpacto() != null
                        && !solicitacaoServicoDTO.getImpacto().equalsIgnoreCase(regraEscalonamentoDTO.getImpacto())
                        && !regraEscalonamentoDTO.getImpacto().isEmpty()) {
                    novaLista.remove(regraEscalonamentoDTO);
                } else if (!temUrgencia && regraEscalonamentoDTO.getUrgencia() != null && !regraEscalonamentoDTO.getUrgencia().isEmpty() || temUrgencia
                        && regraEscalonamentoDTO.getUrgencia() != null
                        && !solicitacaoServicoDTO.getUrgencia().equalsIgnoreCase(regraEscalonamentoDTO.getUrgencia())
                        && !regraEscalonamentoDTO.getUrgencia().isEmpty()) {
                    novaLista.remove(regraEscalonamentoDTO);
                }
    ...
    public Collection<RegraEscalonamentoDTO> findRegraByRequisicaoMudanca(final RequisicaoMudancaDTO requisicaoMudancaDTO, final Integer idTipoGerenciamento)
            throws PersistenceException {
        final List parametro = new ArrayList<>();
        final List listRetorno = new ArrayList<>();
        final StringBuilder sql = new StringBuilder();
            ...
            for (final RegraEscalonamentoDTO regraEscalonamentoDTO : lista) {
                if (!temContrato && regraEscalonamentoDTO.getIdContrato() != null || temContrato && regraEscalonamentoDTO.getIdContrato() != null
                    ...
                    novaLista.remove(regraEscalonamentoDTO);
                        ...
                        && regraEscalonamentoDTO.getImpacto() != null
                        && !requisicaoMudancaDTO.getNivelImpacto().equalsIgnoreCase(regraEscalonamentoDTO.getImpacto())
                        && !regraEscalonamentoDTO.getImpacto().isEmpty()) {
                    novaLista.remove(regraEscalonamentoDTO);
                } else if (!temUrgencia && regraEscalonamentoDTO.getUrgencia() != null && !regraEscalonamentoDTO.getUrgencia().isEmpty() || temUrgencia
                        && regraEscalonamentoDTO.getUrgencia() != null
                        && !requisicaoMudancaDTO.getNivelUrgencia().equalsIgnoreCase(regraEscalonamentoDTO.getUrgencia())
                        && !regraEscalonamentoDTO.getUrgencia().isEmpty()) {
                    novaLista.remove(regraEscalonamentoDTO);
                }
    ...
    public Collection<RegraEscalonamentoDTO> findRegraByProblema(final ProblemaDTO problemaDTO, final Integer idTipoGerenciamento) throws PersistenceException {
        final List parametro = new ArrayList<>();
        final List listRetorno = new ArrayList<>();
        final StringBuilder sql = new StringBuilder();
        boolean temContrato = false, temSolicitante = false, temGrupo = false, temImpacto = false, temUrgencia = false;
            ...
            for (final RegraEscalonamentoDTO regraEscalonamentoDTO : lista) {
                if (!temContrato && regraEscalonamentoDTO.getIdContrato() != null || temContrato && regraEscalonamentoDTO.getIdContrato() != null
                    ...
                    novaLista.remove(regraEscalonamentoDTO);
                        ...
                        && regraEscalonamentoDTO.getImpacto() != null && !problemaDTO.getImpacto().equalsIgnoreCase(regraEscalonamentoDTO.getImpacto())
                        && !regraEscalonamentoDTO.getImpacto().isEmpty()) {
                    novaLista.remove(regraEscalonamentoDTO);
                } else if (!temUrgencia && regraEscalonamentoDTO.getUrgencia() != null && !regraEscalonamentoDTO.getUrgencia().isEmpty() || temUrgencia
                        && regraEscalonamentoDTO.getUrgencia() != null && !problemaDTO.getUrgencia().equalsIgnoreCase(regraEscalonamentoDTO.getUrgencia())
                        && !regraEscalonamentoDTO.getUrgencia().isEmpty()) {
                    novaLista.remove(regraEscalonamentoDTO);
                }
