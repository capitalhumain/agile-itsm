...
public class BICitsmartOperation {
     ...
     */
    public BICitsmartResultRotinaDTO execucaoRotinaAutomatica(final ConexaoBIDTO conexaoBIDTO, final Integer idProcessamentoBatch) throws Exception {

        BICitsmartResultRotinaDTO resultRotina = new BICitsmartResultRotinaDTO();
        resultRotina.setResultado(true);

        final LogImportacaoBIDTO logImportacaoBI = new LogImportacaoBIDTO();
        ...
        try {
            ...
            if (resultRotina.isResultado()) {
                ...
                if (conexaoBIDTO.getTipoImportacao() == null || conexaoBIDTO.getTipoImportacao().equalsIgnoreCase("A")) {
                    ...
                    if (!conexaoBIDTO.getLink().equals("") && !conexaoBIDTO.getLogin().equals("") && !conexaoBIDTO.getSenha().equals("")) {
                        resultRotina = this.rotinaAutoBICitsmart(conexaoBIDTO.getIdConexaoBI(), conexaoBIDTO.getLink(), conexaoBIDTO.getLogin(),
                                conexaoBIDTO.getSenha());
                    } else {
                        resultRotina.setResultado(false);
                        resultRotina.concatMensagem("As informações da conexão não foram preenchidas corretamente para a execução da rotina automática!");
                    }
            ...
            final LogImportacaoBIService logImportacaoBIService = (LogImportacaoBIService) ServiceLocator.getInstance().getService(
                    LogImportacaoBIService.class, null);
    ...
    public BICitsmartResultRotinaDTO rotinaAutoBICitsmart(final Integer idConexao, String url, final String usuario, final String senha) throws Exception {
        ...
        final BICitsmart biCitsmart = new BICitsmart();
        ...
        try {
            ...
            final BICitsmartResultRotinaDTO autenticacao = BICitsmartUtils.autenticacaoComJSON(url, usuario, senha);

            if (autenticacao.isResultado()) {
                biCitsmart.setSessionID(autenticacao.getSessionID());

                final ClientRequest request = new ClientRequest(url + "services/bicitsmart/recuperarTabelas");

                final String biCitsmartJson = new Gson().toJson(biCitsmart);
                ...
                if (response != null && response.getStatus() != 200) {
                    ...
                    final InputStream ioos = new ByteArrayInputStream(response.getEntity().getBytes("UTF-8"));
                    final JsonReader reader = new JsonReader(new InputStreamReader(ioos, "UTF-8"));
                    ...
                    final String xmlString = StringEscapeUtils.unescapeHtml4(response.getEntity());
        ...
        } catch (final NoHttpResponseException nhttpe) {
        ...
        } catch (final JsonSyntaxException jsyn) {
     ...
     * @author rodrigo.acorse
     */
    public BICitsmartResultRotinaDTO testeConexaoClienteBICitsmart(final HttpServletRequest req, final Integer idConexaoBI, String url, final String usuario,
        ...
        try {
            ...
            if (response != null && response.getStatus() != 200) {
                ...
                if (resp != null && resp.getError().getDescription() != null && !resp.getError().getDescription().equals("")) {
                    resultRotina.concatMensagem(UtilI18N.internacionaliza(req, "conexaoBI.testeConexaoErroFalhaConexao") + " "
    ...
    public boolean exportacaoManualBICitsmart(String path) throws Exception {
        ...
        try {
            ...
            if (idConexaoBI == null || idConexaoBI.equals("")) {
                ...
                final String xmlString = StringEscapeUtils.unescapeHtml4(BICitsmartUtils.recuperaXmlTabelasBICitsmart(true));
                ...
                if (xmlString != null && xmlString.length() > 0) {
                    ...
                    path = path + "bi_citsmart_exportacao_" + idConexaoBI + "_" + UtilDatas.getDataAtual() + "_" + c.get(Calendar.HOUR_OF_DAY) + "h"
                            + c.get(Calendar.MINUTE) + ".xml";
    ...
    public boolean importacaoManualBICitsmart(final Integer idConexao, final String xmlPath) throws Exception {
        ...
        try {
            ...
            if (!xmlSource.equals("")) {
                final BICitsmartResultRotinaDTO resultPersistencia = this.persisteDadosBICitsmart(idConexao, xmlSource, true);
     ...
     */
    public BICitsmartResultRotinaDTO persisteDadosBICitsmart(final Integer idConexao, final String xml, final boolean xmlComIdConexao)
            throws PersistenceException {
        final BICitsmartResultRotinaDTO resultRotina = new BICitsmartResultRotinaDTO();
        final TransactionControlerImpl transactionControler = new TransactionControlerImpl(Constantes.getValue("DATABASE_BI_ALIAS"));

        // euler.ramos
        // Declarando variáveis que serão utilizadas na iteração e, como são em sucessivas execuções, estavam onerando o Garbage Collection nas Declarações
        // sucessivas
        ImportInfoRecord importInfoRecord;
        final StringBuilder sqlInsert = new StringBuilder();
        final StringBuilder sqlInsertFields = new StringBuilder();
        final StringBuilder sqlInsertValues = new StringBuilder();
        final StringBuilder sqlUpdate = new StringBuilder();
        final StringBuilder sqlUpdateFields = new StringBuilder();
        ...
        try {
            ...
            final JdbcEngine jdbcEngine = new JdbcEngine(Constantes.getValue("DATABASE_BI_ALIAS"), null);
                ...
                if (importInfoRecord.getTableName() != null && importInfoRecord.getTableName().trim().length() > 0) {
                    ...
                    for (final Iterator it = importInfoRecord.getColFields().iterator(); it.hasNext();) {
                        ...
                        if (field.equalsIgnoreCase("IDCONEXAOBI") && Integer.parseInt(fieldValue) != idConexao) {
                            ...
                            /*
                             * Rodrigo Pecci Acorse - 03/01/2014 10h15 - #128776 Remove o .000 dos campos numéricos para que a persistência seja feita
                             * corretamente.
                             */
                            ...
                            if (importInfoField.isKey()) {
                                ...
                                if (importInfoField.getValueField().trim().equalsIgnoreCase("null")) {
                                    ...
                                    if (typeDB.startsWith("MONEY") || typeDB.startsWith("DOUBLE") || typeDB.startsWith("DECIMAL")
                                            || typeDB.startsWith("NUMERIC") || typeDB.startsWith("NUMBER") || typeDB.startsWith("REAL")
                    ...
                    if (resultRotina.isResultado()) {
                        ...
                        // Monta select final
                        sqlSelect.append("SELECT ").append(nomePrimeiroCampo).append(" FROM ").append(importInfoRecord.getTableName()).append(" WHERE ")
                                .append(sqlWhere);
                        ...
                        if (lst == null || lst.size() == 0) {
                            if (sqlInsertFields.length() > 0 && sqlInsertValues.length() > 0) {
                                ...
                                sqlInsert.append("INSERT INTO ").append(importInfoRecord.getTableName()).append("(").append(sqlInsertFields.toString())
                                        .append(") VALUES (").append(sqlInsertValues.toString()).append(")");
                            ...
                            if (sqlUpdateFields.length() > 0 && sqlWhere.length() > 0) {
                                ...
                                sqlUpdate.append("UPDATE ").append(importInfoRecord.getTableName()).append(" SET ").append(sqlUpdateFields.toString())
                                        .append(" WHERE ").append(sqlWhere.toString());
