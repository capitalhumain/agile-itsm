...
public class NetDiscoverAdvanced implements Runnable {
    ...
    public static void main(final String[] args) throws IOException {
        final NetDiscoverAdvanced netDiscoverAdvanced = new NetDiscoverAdvanced();
        for (int i = 1; i < 255; i++) {
                    ...
                    if (faixaIp.startsWith("file:")) {
                        ...
                        try {
                            ...
                            exec = Runtime.getRuntime().exec(
                                    new String[] {this.getParametroCaminhoNmap(), "-sP", faixaIp.trim(), "-oN", caminhoNetMap + "nmapFile" + i});
    ...
    class PrintStream extends Thread {
        ...
        public PrintStream(final java.io.InputStream is) {
            __is = is;
        }
                ...
                if (linha.length() > 0) {
                    ...
                    if (!fim) {
                        ...
                        if (linha.substring(0, 9).equalsIgnoreCase("Nmap scan")) {
                            ...
                            if (this.pingOK(netMapDto.getIp())) {
                            ...
                            try {
                                ...
                                netMapDto.setNome(aux);
                                netMapDto.setIp(aux.replaceAll("[^0-9.]", ""));
                                if (this.pingOK(netMapDto.getIp())) {
                                    listNetMap.add(netMapDto);
                                }
                            } catch (final Exception e) {}
    ...
    public boolean pingOK(final String ipAddress) {
        InetAddress inet;
        try {
    ...
    private List<NetMapDTO> gravarDados(final List<NetMapDTO> lst) {
        ...
        try {
            final ItemConfiguracaoService itemConfService = (ItemConfiguracaoService) ServiceLocator.getInstance().getService(ItemConfiguracaoService.class,
                    null);
            if (lst != null) {
                for (final NetMapDTO netMapDTO : lst) {
                    ...
                    final List lstStorages = new ArrayList<>();
                    try {
                    ...
                    if (lstStorages != null && lstStorages.size() > 0) {
                        ...
                        if (itemConfiguracaoDto.getIdItemConfiguracao() != null) {
                            ...
                            /*
                             * for (Iterator it = lstStorages.iterator(); it.hasNext();){ SNMPStorageDTO snmpStorageDTO = (SNMPStorageDTO)it.next();
                             * TipoItemConfiguracaoDTO tpItemConfigbean = new
                             * TipoItemConfiguracaoDTO(); if (snmpStorageDTO.getTipo().equalsIgnoreCase(SNMPStorageDTO.FISICA)){
                             ...
                             * caracteristicabean = new CaracteristicaDTO(); ValorDTO valorbean = new ValorDTO(); caracteristicabean.setNome("CAPTION");
                             * caracteristicabean.setTipo("A");
                             * caracteristicabean.setTag("CAPTION"); valorbean.setValorStr(snmpStorageDTO.getDescricao());
                             * caracteristicabean.setValor(valorbean);
                             * lstCaracteristica.add(caracteristicabean);
                             * caracteristicabean = new CaracteristicaDTO(); valorbean = new ValorDTO(); caracteristicabean.setNome("DESCRIPTION");
                             * caracteristicabean.setTipo("A");
                             * caracteristicabean.setTag("DESCRIPTION"); valorbean.setValorStr(snmpStorageDTO.getDescricao());
                             * caracteristicabean.setValor(valorbean);
                             * lstCaracteristica.add(caracteristicabean);
                             * caracteristicabean = new CaracteristicaDTO(); valorbean = new ValorDTO(); caracteristicabean.setNome("CAPACITY");
                             * caracteristicabean.setTipo("A");
                             * caracteristicabean.setTag("CAPACITY"); valorbean.setValorStr("" + (((snmpStorageDTO.getSize() *
                             * snmpStorageDTO.getAllocationUnits()) / 1024) / 1024));
                             * caracteristicabean.setValor(valorbean); lstCaracteristica.add(caracteristicabean);
                             * caracteristicabean = new CaracteristicaDTO(); valorbean = new ValorDTO(); caracteristicabean.setNome("USED");
                             * caracteristicabean.setTipo("A");
                             * caracteristicabean.setTag("USED"); valorbean.setValorStr("" + (((snmpStorageDTO.getUsed() * snmpStorageDTO.getAllocationUnits())
                             * / 1024) / 1024));
                             * caracteristicabean.setValor(valorbean); lstCaracteristica.add(caracteristicabean);
                             * tpItemConfigbean.setCaracteristicas(lstCaracteristica); lstTipoItemConfi.add(tpItemConfigbean); }else{
                             * tpItemConfigbean.setTag("STORAGES");
                             * tpItemConfigbean.setNome("STORAGES");
                             * List<CaracteristicaDTO> lstCaracteristica = new ArrayList<CaracteristicaDTO>(); CaracteristicaDTO caracteristicabean = null;
                             * caracteristicabean = new CaracteristicaDTO(); ValorDTO valorbean = new ValorDTO(); caracteristicabean.setNome("NAME");
                             * caracteristicabean.setTipo("A");
                             ...
                             * caracteristicabean.setValor(valorbean);
                             * lstCaracteristica.add(caracteristicabean);
                             * caracteristicabean = new CaracteristicaDTO(); valorbean = new ValorDTO(); caracteristicabean.setNome("DISKSIZE");
                             * caracteristicabean.setTipo("A");
                             * caracteristicabean.setTag("DISKSIZE"); valorbean.setValorStr("" + (((snmpStorageDTO.getSize() *
                             * snmpStorageDTO.getAllocationUnits()) / 1024) / 1024));
                             * caracteristicabean.setValor(valorbean); lstCaracteristica.add(caracteristicabean);
                             * caracteristicabean = new CaracteristicaDTO(); valorbean = new ValorDTO(); caracteristicabean.setNome("USED");
                             * caracteristicabean.setTipo("A");
                            ...
                            final List<ItemConfiguracaoDTO> list = new ArrayList<ItemConfiguracaoDTO>();
                            ...
                            for (final ItemConfiguracaoDTO itemConfiguracaoDTO2 : list) {
    ...
    /*
     * public static String getSNMPSystem(String ipAddr, NetMapDTO netMapDTO) { String strXML = getSNMPInfo("http://10.0.1.94:3000/transaction/system/1/" +
     * ipAddr); DocumentBuilderFactory factory =
     * DocumentBuilderFactory.newInstance(); try { InputStream ioos = new ByteArrayInputStream(strXML.getBytes("UTF-8")); DocumentBuilder builder =
     * factory.newDocumentBuilder(); Document doc = null;
     * doc = builder.parse(ioos); Node noRoot = null; noRoot = doc.getChildNodes().item(0); try{ for(int j = 0; j < noRoot.getChildNodes().getLength(); j++){
     * Node noLookup =
     * noRoot.getChildNodes().item(j); if (noLookup == null || noLookup.getNodeName() == null){ continue; } if(noLookup.getNodeName().equals("#text")) continue;
     * for(int x = 0; x <
     * noLookup.getChildNodes().getLength(); x++){ Node noItem = noLookup.getChildNodes().item(x); if(noItem.getNodeName().equals("#text")) continue; for(int z
     * = 0; z <
     * noItem.getChildNodes().getLength(); z++){ Node noItemX = noItem.getChildNodes().item(z); if(noItemX.getNodeName().equals("#text")) continue;
     * //System.out.println("NodeValue2: ===> " +
     * noItemX.getNodeName() + ":" + noItemX.getTextContent()); if (noItemX.getNodeName().equalsIgnoreCase("descr")){ String str = noItemX.getTextContent(); int
     * index = str.indexOf("Software:");
     * String nomeHard = ""; String nomeSO = ""; if (index > -1){ int indexHard = str.indexOf("Hardware:"); if (indexHard > -1){ nomeHard =
     * str.substring(indexHard + 9, index - 3); } nomeSO =
     * str.substring(index + 9); } netMapDTO.setHardware(nomeHard.trim()); netMapDTO.setSistemaoper(nomeSO.trim()); } if
     * (noItemX.getNodeName().equalsIgnoreCase("name")){
     * netMapDTO.setNome(noItemX.getTextContent()); } if (noItemX.getNodeName().equalsIgnoreCase("upTime")){ netMapDTO.setUptime(noItemX.getTextContent()); } }
     * } } }catch(Exception e){
     * e.printStackTrace(); } } catch (ParserConfigurationException e) { e.printStackTrace(); } catch (SAXException e) { e.printStackTrace(); } catch
     * (IOException e) { e.printStackTrace(); }
     * return ""; }
     * public static List getSNMPStorages(String ipAddr, NetMapDTO netMapDTO) { String strXML = getSNMPInfo("http://10.0.1.94:3000/transaction/storage/1/" +
     * ipAddr); DocumentBuilderFactory factory =
     * DocumentBuilderFactory.newInstance(); List lstStorages = new ArrayList<>(); try { InputStream ioos = new ByteArrayInputStream(strXML.getBytes("UTF-8"));
     * DocumentBuilder builder =
     * factory.newDocumentBuilder(); Document doc = null; doc = builder.parse(ioos); Node noRoot = null; noRoot = doc.getChildNodes().item(0); try{ for(int j =
     * 0; j <
     * noRoot.getChildNodes().getLength(); j++){ Node noLookup = noRoot.getChildNodes().item(j); if (noLookup == null || noLookup.getNodeName() == null){
     * continue; }
     * if(noLookup.getNodeName().equals("#text")) continue; for(int x = 0; x < noLookup.getChildNodes().getLength(); x++){ Node noItem =
     * noLookup.getChildNodes().item(x);
     * if(noItem.getNodeName().equals("#text")) continue; SNMPStorageDTO snmpStorageDTO = new SNMPStorageDTO(); snmpStorageDTO.setAllocationUnits(0); for(int z
     * = 0; z <
     * noItem.getChildNodes().getLength(); z++){ Node noItemX = noItem.getChildNodes().item(z); if(noItemX.getNodeName().equals("#text")) continue;
     * //System.out.println("NodeValue2: ===> " +
     * noItemX.getNodeName() + ":" + noItemX.getTextContent()); if (noItemX.getNodeName().equalsIgnoreCase("storageDescr")){
     * snmpStorageDTO.setDescricao(noItemX.getTextContent());
     * snmpStorageDTO.setTipo(SNMPStorageDTO.DISCO); if (snmpStorageDTO.getDescricao().startsWith("Physical Memory")){
     * snmpStorageDTO.setTipo(SNMPStorageDTO.FISICA); } } if
     * (noItemX.getNodeName().equalsIgnoreCase("storageAllocationUnits")){ String str = noItemX.getTextContent(); if (str != null){ Integer int1 = 0; try{ int1
     * = Integer.parseInt(str.trim());
     * snmpStorageDTO.setAllocationUnits(int1); }catch(Exception e){ } } } if (noItemX.getNodeName().equalsIgnoreCase("storageSize")){ String str =
     * noItemX.getTextContent(); if (str != null){ Long
     * int1 = (long) 0; try{ int1 = Long.parseLong(str.trim()); snmpStorageDTO.setSize(int1); }catch(Exception e){ } } } if
     * (noItemX.getNodeName().equalsIgnoreCase("storageUsed")){ String str =
     * noItemX.getTextContent(); if (str != null){ Long int1 = (long) 0; try{ int1 = Long.parseLong(str.trim()); snmpStorageDTO.setUsed(int1); }catch(Exception
     * e){ } } } }
     * lstStorages.add(snmpStorageDTO); } } }catch(Exception e){ e.printStackTrace(); } } catch (ParserConfigurationException e) { e.printStackTrace(); } catch
     * (SAXException e) { e.printStackTrace();
     * } catch (IOException e) { e.printStackTrace(); }
     * return lstStorages; }
     * public static String getSNMPInfo(String strurl) {
     * try { URL url = new URL(strurl);
     * URLConnection urlConnection = url.openConnection(); urlConnection.setRequestProperty("accept", "text/xml"); BufferedReader xml = new BufferedReader(new
     * InputStreamReader(
     * urlConnection.getInputStream())); String str; String strFinal = ""; while ((str = xml.readLine()) != null) { if (!strFinal.equalsIgnoreCase("")){
     * strFinal = strFinal + ""; } strFinal = strFinal
    ...
    public void run() {
        try {
            final NetDiscoverAdvanced netDiscover = new NetDiscoverAdvanced();
    ...
    private String[] getParametroFaixaIPNmap() throws ServiceException, LogicException, Exception {
        ...
        final String strFaixaIp = ParametroUtil.getValorParametroCitSmartHashMap(Enumerados.ParametroSistema.FaixaIp, " ");
