...
public class ServicoDao extends CrudDaoDefaultImpl {
    ...
    public Collection<ServicoDTO> listaQuantidadeServicoAnalitico(final ServicoDTO servicoDTO) throws Exception {
         ...
         * @author thyen.chang
         */
        final boolean seLimita = servicoDTO.getTopList() != 0;
        ...
        if (CITCorporeUtil.SGBD_PRINCIPAL.toUpperCase().equals(SQLConfig.ORACLE)) {
            ...
            parametro.add(this.transformaHoraFinal(servicoDTO.getDataFim()));
        }
         ...
         */
        if (seLimita
                && (CITCorporeUtil.SGBD_PRINCIPAL.toUpperCase().equals(SQLConfig.MYSQL) || CITCorporeUtil.SGBD_PRINCIPAL.toUpperCase().equals(
                        SQLConfig.POSTGRESQL))) {
            sql.append("LIMIT ?");
            parametro.add(servicoDTO.getTopList());
        }
        ...
        if (lista != null && !lista.isEmpty()) {
            listaQuantidadeServicoAnalitico = engine.listConvertion(ServicoDTO.class, lista, listRetorno);
    ...
    public Collection findByIdTipoDemandaAndIdContrato(final Integer idTipoDemanda, final Integer idContrato, final Integer idCategoria) throws Exception {
        ...
        final Object[] objs = new Object[] {dataAtual};
        ...
        sql += " idServico IN (SELECT idservico FROM servicocontrato WHERE idcontrato = " + idContrato
                + " AND (datafim is null OR datafim > ?) AND ((deleted IS NULL) OR (UPPER(deleted) = 'N'))) AND";
        ...
        final List listaR = this.execSQL(sql, objs);
        return this.listConvertion(ServicoDTO.class, listaR, listaNomes);
    }

    public Collection findByNomeAndContratoAndTipoDemandaAndCategoria(final Integer idTipoDemanda, final Integer idContrato, final Integer idCategoria,
     ...
     * @throws Exception
     */
    public String retornaSiglaPorIdOs(final Integer idOs) throws Exception {
        ...
        final Collection<ServicoDTO> listDeFaturas = super.listConvertion(ServicoDTO.class, lstDados, fields);

        if (listDeFaturas != null && !listDeFaturas.isEmpty()) {
            for (final ServicoDTO servico : listDeFaturas) {
     ...
     * @throws Exception
     */
    public Collection findByNome(final ServicoDTO servicoDTO) throws Exception {
        final List<Condition> condicao = new ArrayList<>();
        final List<Order> ordenacao = new ArrayList<>();
    ...
    public ServicoDTO findByIdServico(final Integer idServico) throws Exception {
        ...
        final String sql = "   select nometiposervico, nomeservico, nomecategoriaservico, idServico from servico "
        ...
        if (list != null && !list.isEmpty()) {
            return (ServicoDTO) this.listConvertion(this.getBean(), list, fields).get(0);
        } else {
            return null;
        }
    }
    ...
    public Collection<ServicoDTO> findByServico(final Integer idServico, final String nome) throws Exception {
        ...
        final String sql = "   select nometiposervico, nomeservico, nomecategoriaservico, idServico from servico "
                + "inner join tiposervico  on servico.idtiposervico = tiposervico.idtiposervico "
                + "inner join categoriaservico on servico.idcategoriaservico = categoriaservico.idcategoriaservico "
                + "where servico.deleted is null and situacao = 'A' and servico.idServico = ? and (dispportal = 'Y' or dispportal = 'S') and servico.nomeservico like '%"
                + nome + "%' order by nomecategoriaservico";
        parametro.add(idServico);
        list = this.execSQL(sql, parametro.toArray());
        fields.add("nomeTipoServico");
        fields.add("nomeServico");
    ...
    public Collection<ServicoDTO> findByIdTemplate(final Integer idTemplate) throws Exception {
    ...
    public ServicoDTO findById(final Integer parm) throws Exception {
        final List<Condition> condicao = new ArrayList<>();
        final List<Order> ordenacao = new ArrayList<>();
        condicao.add(new Condition("idServico", "=", parm));
        final ArrayList<ServicoDTO> lista = (ArrayList<ServicoDTO>) super.findByCondition(condicao, ordenacao);
    ...
    public Collection<ServicoDTO> listAtivosDiferenteContrato(final ServicoDTO servicoDto) throws Exception {
        ...
        if (list != null && !list.isEmpty()) {
            final Collection<ServicoDTO> listAtivosDiferenteContrato = this.listConvertion(ServicoDTO.class, list, listRetorno);
