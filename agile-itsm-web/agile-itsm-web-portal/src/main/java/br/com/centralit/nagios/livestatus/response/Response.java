...
public class Response {
    ...
    public Response(final LSQuery pQuery) {
    ...
    public Map<String, String> getMapColumnsAndValue() {
        ...
        for (int k = 0; k < qdlines; k++) {
            final String[] cols = string_list.get(k).split(LivestatusSeparator.SEP2());
    ...
    public String[] getHeaders_and_columns_asArrayString() {
        ...
        final String[] hc = new String[qdlines * string_list.get(0).split(LivestatusSeparator.SEP2()).length];
        for (int k = 0; k < qdlines; k++) {
            final String[] cols = string_list.get(k).split(LivestatusSeparator.SEP2());
            if (cols.length != string_array_headers.length) {
    ...
    public String asString() {
        ...
        if (string_list != null && !string_list.isEmpty()) {
            for (final String name : string_list) {
    ...
    public void add(final String line) {
        final boolean hasColumns = query.hasColumns() && query.hasColumnHeaders() || !query.hasColumns();
