...
public class RelatorioQuantitativoPorServico extends AjaxFormAction {
	...
	public void load(DocumentHTML document, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		for(Enumerados.TopListEnum valor : Enumerados.TopListEnum.values()){
			if( valor.equals(Enumerados.TopListEnum.TOPTODOS) )
				document.getSelectById("numeroRegistros").addOption("", UtilI18N.internacionaliza(request, valor.getNomeTopList()));
			else
				document.getSelectById("numeroRegistros").addOption(valor.getValorTopList().toString(), UtilI18N.internacionaliza(request, valor.getNomeTopList()));
		}
