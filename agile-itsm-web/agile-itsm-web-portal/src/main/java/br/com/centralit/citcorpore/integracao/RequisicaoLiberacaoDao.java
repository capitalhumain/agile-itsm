...
public class RequisicaoLiberacaoDao extends CrudDaoDefaultImpl {
    ...
    public void updateFase(final Integer idRequisicao, final String fase) {
        final StringBuilder sql = new StringBuilder();
        sql.append("UPDATE " + this.getTableName() + " SET fase = ? WHERE (idliberacao = ?)");
    ...
    public Collection<RequisicaoLiberacaoDTO> listaRequisicaoLiberacaoPorCriterios(final PesquisaRequisicaoLiberacaoDTO pesquisaRequisicaoLiberacaoDto)
        ...
        if (pesquisaRequisicaoLiberacaoDto.getDataInicio() != null && pesquisaRequisicaoLiberacaoDto.getDataFim() != null) {
            ...
            parametros.add(this.transformaHoraFinal(pesquisaRequisicaoLiberacaoDto.getDataFim()));
        }

        if (pesquisaRequisicaoLiberacaoDto.getDataInicioFechamento() != null
                && !StringUtils.equalsIgnoreCase(pesquisaRequisicaoLiberacaoDto.getDataInicioFechamento().toString(), "1970-01-01")) {
            sql.append("AND (l.datahoraconclusao BETWEEN ? AND ?) ");
            parametros.add(pesquisaRequisicaoLiberacaoDto.getDataInicioFechamento());
            parametros.add(this.transformaHoraFinal(pesquisaRequisicaoLiberacaoDto.getDataFimFechamento()));
        }
    ...
    public List<RequisicaoLiberacaoDTO> listLiberacaoByIdItemConfiguracao(final Integer idItemConfiguracao) throws Exception {
        final StringBuilder sql = new StringBuilder();
