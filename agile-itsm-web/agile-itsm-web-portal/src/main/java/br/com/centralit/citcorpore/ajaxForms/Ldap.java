...
public class Ldap extends AjaxFormAction {
    ...
    public void load(final DocumentHTML document, final HttpServletRequest request, final HttpServletResponse response) throws Exception {
        ...
        document.executeScript("addLinhaTabelaAtributosLdap(" + Enumerados.ParametroSistema.LDAP_URL.id() + ",'"
                + Enumerados.ParametroSistema.LDAP_URL.getCampoParametroInternacionalizado(request) + "','"
        ...
        document.executeScript("addLinhaTabelaAtributosLdap(" + Enumerados.ParametroSistema.DOMINIO_AD.id() + ",'"
                + Enumerados.ParametroSistema.DOMINIO_AD.getCampoParametroInternacionalizado(request) + "','"
        ...
        document.executeScript("addLinhaTabelaAtributosLdap(" + Enumerados.ParametroSistema.LDAP_SUBDOMINIO.id() + ",'"
                + Enumerados.ParametroSistema.LDAP_SUBDOMINIO.getCampoParametroInternacionalizado(request) + "','"
        ...
        document.executeScript("addLinhaTabelaAtributosLdap(" + Enumerados.ParametroSistema.LDAD_SUFIXO_DOMINIO.id() + ",'"
                + Enumerados.ParametroSistema.LDAD_SUFIXO_DOMINIO.getCampoParametroInternacionalizado(request) + "','"
        ...
        document.executeScript("addLinhaTabelaAtributosLdap(" + Enumerados.ParametroSistema.LOGIN_AD.id() + ",'"
                + Enumerados.ParametroSistema.LOGIN_AD.getCampoParametroInternacionalizado(request) + "','"
        ...
        document.executeScript("addLinhaTabelaAtributosLdap(" + Enumerados.ParametroSistema.SENHA_AD.id() + ",'"
                + Enumerados.ParametroSistema.SENHA_AD.getCampoParametroInternacionalizado(request) + "','"
        ...
        document.executeScript("addLinhaTabelaAtributosLdap(" + Enumerados.ParametroSistema.LDAP_FILTRO.id() + ",'"
                + Enumerados.ParametroSistema.LDAP_FILTRO.getCampoParametroInternacionalizado(request) + "','"
        ...
        document.executeScript("addLinhaTabelaAtributosLdap(" + Enumerados.ParametroSistema.LDAP_ATRIBUTO.id() + ",'"
                + Enumerados.ParametroSistema.LDAP_ATRIBUTO.getCampoParametroInternacionalizado(request) + "','"
                + StringUtils.replace(ParametroUtil.getValorParametroCitSmartHashMap(Enumerados.ParametroSistema.LDAP_ATRIBUTO, ""), "'", "\'") + " ')");
        document.executeScript("addLinhaTabelaAtributosLdap("
                + Enumerados.ParametroSistema.LDAP_SN_LAST_NAME.id()
                + ",'"
                + Enumerados.ParametroSistema.LDAP_SN_LAST_NAME.getCampoParametroInternacionalizado(request)
                + "','"
                + Internacionalizar.internacionalizaOptionSN(request, "restore",
        ...
        document.executeScript("addLinhaTabelaAtributosLdap(" + Enumerados.ParametroSistema.ID_PERFIL_ACESSO_DEFAULT.id() + ",'"
                + Enumerados.ParametroSistema.ID_PERFIL_ACESSO_DEFAULT.getCampoParametroInternacionalizado(request) + "','"
        ...
        document.executeScript("addLinhaTabelaAtributosLdap(" + Enumerados.ParametroSistema.ID_GRUPO_PADRAO_LDAP.id() + ",'"
                + Enumerados.ParametroSistema.ID_GRUPO_PADRAO_LDAP.getCampoParametroInternacionalizado(request) + "','"
                + StringUtils.replace(ParametroUtil.getValorParametroCitSmartHashMap(Enumerados.ParametroSistema.ID_GRUPO_PADRAO_LDAP, ""), "'", "\'") + " ')");
        document.executeScript("addLinhaTabelaAtributosLdap("
                + Enumerados.ParametroSistema.NUMERO_COLABORADORES_CONSULTA_AD.id()
                + ",'"
                + Enumerados.ParametroSistema.NUMERO_COLABORADORES_CONSULTA_AD.getCampoParametroInternacionalizado(request)
                + "','"
                + StringUtils.replace(ParametroUtil.getValorParametroCitSmartHashMap(Enumerados.ParametroSistema.NUMERO_COLABORADORES_CONSULTA_AD, ""), "'",
                        "\'") + " ')");
        document.executeScript("addLinhaTabelaAtributosLdap("
                + Enumerados.ParametroSistema.LDAP_MOSTRA_BOTAO.id()
                + ",'"
                + StringUtils.replace(Enumerados.ParametroSistema.LDAP_MOSTRA_BOTAO.getCampoParametroInternacionalizado(request), "'", "\\'")
                + "','"
                + Internacionalizar.internacionalizaOptionSN(request, "restore",
                        StringUtils.replace(ParametroUtil.getValorParametroCitSmartHashMap(Enumerados.ParametroSistema.LDAP_MOSTRA_BOTAO, "N"), "\'", "\\'"))
                        + " ')");
        document.executeScript("addLinhaTabelaAtributosLdap("
                + Enumerados.ParametroSistema.LDAP_OPEN_LDAP.id()
                + ",'"
                + StringUtils.replace(Enumerados.ParametroSistema.LDAP_OPEN_LDAP.getCampoParametroInternacionalizado(request), "'", "\\'")
                + "','"
                + Internacionalizar.internacionalizaOptionSN(request, "restore",
    ...
    public void save(final DocumentHTML document, final HttpServletRequest request, final HttpServletResponse response) throws Exception {
        ...
        final LdapDTO ldap = (LdapDTO) document.getBean();
        ...
        if (listaAdUserDto != null && !listaAdUserDto.isEmpty()) {
            ...
            for (final ADUserDTO adUserDto : listaAdUserDto) {
                ...
                if (adUserDto != null && adUserDto.isAtivo()) {
                    document.alert(UtilI18N.internacionaliza(request, "ldap.conexaocom") + getDisponivel(request, adUserDto.getServer()) + " "
                            + UtilI18N.internacionaliza(request, "ldap.conexaorealizadasucesso") + "\n  \n sAMAccountName: "
                            + getDisponivel(request, adUserDto.getsAMAccountName()) + "\n E-mail: " + getDisponivel(request, adUserDto.getMail()) + "\n CN: "
                            + getDisponivel(request, adUserDto.getCN()) + "\n SN: " + getDisponivel(request, adUserDto.getSN()) + "\n DN: "
                            + getDisponivel(request, adUserDto.getDN()) + "\n Display Name: " + getDisponivel(request, adUserDto.getDisplayName()));
                } else {
                    document.alert(UtilI18N.internacionaliza(request, "ldap.conexaocom") + " " + getDisponivel(request, adUserDto.getServer()) + " "
                            + UtilI18N.internacionaliza(request, "ldap.conexaofalhou"));
                }
            }
        } else {
     ...
     * @throws Exception
     */
    public void sincronizaLDAP(final DocumentHTML document, final HttpServletRequest request, final HttpServletResponse response) throws Exception {
