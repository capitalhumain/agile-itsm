...
public class RequisicaoMudancaDao extends CrudDaoDefaultImpl {
    ...
    public void updateFase(final Integer idRequisicao, final String fase) {
        final StringBuilder sql = new StringBuilder();
        sql.append("UPDATE " + this.getTableName() + " SET fase = ? WHERE (idrequisicaomudanca = ?)");
        final Object[] params = {fase, idRequisicao};
        try {
    ...
    public Collection findByIdRequisicaoMudancaEDataFim(final Integer idRequisicaoMudanca) throws Exception {
        final List parametro = new ArrayList<>();
        final List fields = new ArrayList<>();
        List list = new ArrayList<>();
        final StringBuilder sql = new StringBuilder();
    ...
    public List<RequisicaoMudancaDTO> listProblemaByIdSolicitacao(final RequisicaoMudancaDTO bean) throws Exception {
        final List parametro = new ArrayList<>();
        final List fields = new ArrayList<>();
        List list = new ArrayList<>();
        String sql = " select  mud.idrequisicaomudanca , mud.titulo, empcriadopor.nome, empmudpri.nome, mud.status, sol.idSolicitacaoServico, ser.nomeServico  from requisicaomudanca  mud  "
                + "	inner join  empregados empcriadopor on mud.idsolicitante = empcriadopor.idempregado  "
                + "	inner join  empregados empmudpri on mud.idproprietario = empmudpri.idempregado  "
                ...
                + "	inner join servico ser on ser.idservico = sercont.idservico  ";
    ...
    public List<RequisicaoMudancaDTO> listMudancaByIdItemConfiguracao(final Integer idItemConfiguracao) throws Exception {
        final String sql = " select  mud.idrequisicaomudanca, mud.titulo, mud.descricao , mud.idproprietario, mud.idsolicitante, "
                // + " mud.nivelimpacto,mud.idCategoriaMudanca " +
                + " mud.nivelimpacto,mud.idTipoMudanca " + ", mud.motivo, mud.nivelImportanciaNegocio, mud.status, mud.tipo , emp.nome, us.nome , "
                ...
                + " inner join empregados emp on emp.idempregado = mud.idsolicitante " + " inner join usuario us on us.idusuario = mud.idproprietario "
    ...
    public Collection<RequisicaoMudancaDTO> listaMudancasPorBaseConhecimento(final RequisicaoMudancaDTO mudanca) throws Exception {
        ...
        if (list != null && !list.isEmpty()) {
            final Collection<RequisicaoMudancaDTO> listaMudancaPorBaseConhecimento = this.listConvertion(RequisicaoMudancaDTO.class, list, listRetorno);
    ...
    public Collection<RequisicaoMudancaDTO> quantidadeMudancaPorBaseConhecimento(final RequisicaoMudancaDTO mudanca) throws Exception {
        ...
        final List list = this.execSQL(sql.toString(), parametro.toArray());
        if (list != null && !list.isEmpty()) {
            final Collection<RequisicaoMudancaDTO> listaQuantidadeMudancaPorBaseConhecimento = this.listConvertion(RequisicaoMudancaDTO.class, list,
                    listRetorno);
     ...
     */
    public List<RequisicaoMudancaDTO> findByConhecimento(final BaseConhecimentoDTO baseConhecimentoDto) throws Exception {
        final List parametro = new ArrayList<>();
        final List listRetorno = new ArrayList<>();
        List list = new ArrayList<>();
        final StringBuilder sql = new StringBuilder();
    ...
    public List<RequisicaoMudancaDTO> findByPeriodoAndSituacao(final Date dataInicial, final Date dataFinal, final String situacao) throws Exception {
        final List parametro = new ArrayList<>();
        final List listRetorno = new ArrayList<>();
        List list = new ArrayList<>();
        final StringBuilder sql = new StringBuilder();
    ...
    public Collection<PesquisaRequisicaoMudancaDTO> listRequisicaoMudancaByCriterios(final PesquisaRequisicaoMudancaDTO pesquisaRequisicaoMudancaDto)
        ...
        if (CITCorporeUtil.SGBD_PRINCIPAL.trim().equalsIgnoreCase("ORACLE")) {
            ...
            if (pesquisaRequisicaoMudancaDto.getDataInicio() != null) {
                ...
                sql.append(" where requisicaomudanca.datahorainicio BETWEEN TO_DATE('" + UtilDatas.dateToSTR(pesquisaRequisicaoMudancaDto.getDataInicio())
                        + "', 'DD/MM/YYYY ')  and TO_DATE('" + UtilDatas.dateToSTR(pesquisaRequisicaoMudancaDto.getDataFim()) + "', 'DD/MM/YYYY ') ");
            ...
            if (pesquisaRequisicaoMudancaDto.getDataInicio() != null) {
                // Adaptação realizada passando o timestamp concatenado no
                // próprio sqp para poder colocar aspas simples na pesquisa
                sql.append(" where requisicaomudanca.datahorainicio BETWEEN  '" + UtilDatas.dateToSTR(pesquisaRequisicaoMudancaDto.getDataInicio())
                        + " 00:00:00'   and '" + UtilDatas.dateToSTR(pesquisaRequisicaoMudancaDto.getDataFim()) + " 23:59:59' ");
        ...
        if (list != null && !list.isEmpty()) {
            ...
            final Collection<PesquisaRequisicaoMudancaDTO> listRequisicaoMudancaByCriterios = this.listConvertion(PesquisaRequisicaoMudancaDTO.class, list,
                    listRetorno);
    ...
    public boolean verificarSeRequisicaoMudancaPossuiTipoMudanca(final Integer idTipoMudanca) throws Exception {
        final List parametro = new ArrayList<>();
        final String sql = "select idrequisicaomudanca from " + this.getTableName() + " where idtipomudanca = ? ";
    ...
    public RequisicaoMudancaDTO findByIdInstanciaFluxo(final Integer idInstanciaFluxo) throws Exception {
    ...
    public Collection listaIdETituloMudancasPorPeriodo(final RequisicaoMudancaDTO requisicaoMudancaDTO) throws Exception {
        ...
        if (CITCorporeUtil.SGBD_PRINCIPAL.toUpperCase().equals(SQLConfig.ORACLE)) {
            ...
            parametro.add(this.transformaHoraFinal(requisicaoMudancaDTO.getDataFim()));
        }
        ...
        listRetorno.add("titulo");
        if (lista != null && !lista.isEmpty()) {
            listResultado = engine.listConvertion(RequisicaoMudancaDTO.class, lista, listRetorno);
        }
        return listResultado;
    }

    public Collection listaMudancaIncidente(final RequisicaoMudancaDTO requisicaoMudancaDTO) throws Exception {
        final List listRetorno = new ArrayList<>();
        final List parametro = new ArrayList<>();
        List listResultado = new ArrayList<>();
        final StringBuilder sql = new StringBuilder();
    ...
    public Collection listaMudancaServico(final RequisicaoMudancaDTO requisicaoMudancaDTO) throws Exception {
        final List listRetorno = new ArrayList<>();
        final List parametro = new ArrayList<>();
        List listResultado = new ArrayList<>();
        final StringBuilder sql = new StringBuilder();
    ...
    public Collection listaMudancaProblema(final RequisicaoMudancaDTO requisicaoMudancaDTO) throws Exception {
        final List listRetorno = new ArrayList<>();
        final List parametro = new ArrayList<>();
        List listResultado = new ArrayList<>();
        final StringBuilder sql = new StringBuilder();
    ...
    public Collection listaMudancaConhecimento(final RequisicaoMudancaDTO requisicaoMudancaDTO) throws Exception {
        final List listRetorno = new ArrayList<>();
        final List parametro = new ArrayList<>();
        List listResultado = new ArrayList<>();
        final StringBuilder sql = new StringBuilder();
    ...
    public Collection listaMudancaItemConfiguracao(final RequisicaoMudancaDTO requisicaoMudancaDTO) throws Exception {
        final List listRetorno = new ArrayList<>();
        final List parametro = new ArrayList<>();
        List listResultado = new ArrayList<>();
        final StringBuilder sql = new StringBuilder();
    ...
    public Collection listaQuantidadeMudancaPorPeriodo(final RequisicaoMudancaDTO requisicaoMudancaDTO) throws Exception {
        ...
        if (lista != null && !lista.isEmpty()) {
            listaQuantidadeMudancaPorPeriodo = engine.listConvertion(RequisicaoMudancaDTO.class, lista, listRetorno);
        }
    ...
    public Collection listaQuantidadeMudancaPorSolicitante(final RequisicaoMudancaDTO requisicaoMudancaDTO) throws Exception {
        ...
        if (lista != null && !lista.isEmpty()) {
            listaQuantidadeMudancaPorSolicitante = engine.listConvertion(RequisicaoMudancaDTO.class, lista, listRetorno);
        }
    ...
    public Collection listaQuantidadeMudancaPorImpacto(final RequisicaoMudancaDTO requisicaoMudancaDTO) throws Exception {
        ...
        if (lista != null && !lista.isEmpty()) {
            listaQuantidadeMudancaPorImpacto = engine.listConvertion(RequisicaoMudancaDTO.class, lista, listRetorno);
        }
    ...
    public Collection listaQuantidadeMudancaPorProprietario(final RequisicaoMudancaDTO requisicaoMudancaDTO) throws Exception {
        ...
        if (lista != null && !lista.isEmpty()) {
            listaQuantidadeMudancaPorProprietario = engine.listConvertion(RequisicaoMudancaDTO.class, lista, listRetorno);
        }
    ...
    public Collection listaQuantidadeMudancaPorStatus(final RequisicaoMudancaDTO requisicaoMudancaDTO) throws Exception {
        ...
        if (lista != null && !lista.isEmpty()) {
            listaQuantidadeMudancaPorStatus = engine.listConvertion(RequisicaoMudancaDTO.class, lista, listRetorno);
        }
        return listaQuantidadeMudancaPorStatus;
    }

    public Collection listaMudancaGrupo(final RequisicaoMudancaDTO requisicaoMudancaDTO) throws Exception {
        final List listRetorno = new ArrayList<>();
        final List parametro = new ArrayList<>();
        List listResultado = new ArrayList<>();
        final StringBuilder sql = new StringBuilder();
    ...
    public Collection listSolicitacoesByRegra() throws Exception {
        ...
        sql.append("SELECT distinct (s.idrequisicaomudanca), "
                ...
                + " FROM requisicaomudanca s   " + " INNER JOIN contratos contratos on contratos.idcontrato = s.idcontrato "
