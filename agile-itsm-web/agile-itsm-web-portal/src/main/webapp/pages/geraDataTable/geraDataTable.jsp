...
var intervalo;
	...
	<script type="text/javascript" src="${ctx}/pages/painel/jquery.ui.position.js"></script>
		...
		<style type="text/css" title="currentStyle">
			@import "${ctx}/template_new/js/DataTables/css/jquery.dataTables.css";
			@import "${ctx}/template_new/js/DataTables/css/demo_page.css";
			@import "${ctx}/template_new/js/DataTables/css/demo_table.css";
		</style>
    		...
    		function gerarPainel(){
    			...
    			//if(validaData(dataInicio, dataFim)){
    				if (document.formParametros.validate())
    				{
    					...
    					fecharTelaParm();
    	                $.getJSON("${ctx}/pages/geraDataTable/geraDataTable.load?parmOK=true&idConsulta=<%=idConsulta%>&" + queryStr, function(mps) {
    	                	$('#output').html( '<table cellpadding="0" cellspacing="0" border="0" width="100%" heigth="100%" class="display" id="example"></table>' );
    	                    $('#example').dataTable( {
    	                        "aaData": mps,
    	                        "aoColumns": <%=textJSON_Columns%>
    	                    } );
    	                });
    				}
    			//}
