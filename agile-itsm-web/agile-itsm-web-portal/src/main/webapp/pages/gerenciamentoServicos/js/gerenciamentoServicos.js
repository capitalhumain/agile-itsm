...
$(function() {
    ...
    $('#idSolicitante').select2({
        ...
        ajax: {
            // How long the user has to pause their typing before sending the
            // next request
            quietMillis: 150,
            ...
            results: function(data, page) {
                // Used to determine whether or not there are more results
                // available,
                // and if requests for more data should be sent in the infinite
                // scrolling
                return {
                    results: data
                };
    ...
    $('#tarefaAtual')
        .select2({
            // Does the user have to enter any data before sending
            // the ajax request
            minimumInputLength: 0,
            allowClear: true,
            placeholder: i18n_message("citcorpore.comum.selecione"),
            ajax: {
                // How long the user has to pause their typing
                ...
                results: function(data, page) {
                    // Used to determine whether or not there are
                    // more results available,
                    // and if requests for more data should be sent
                    // in the infinite scrolling
                    return {
                        results: data
                    };
                }
...
function inicializaPopover() {
    ...
    $('.informacoesSolicitante').popover({
        placement: 'top',
        animation: true,
        trigger: 'click',
        html: true
            ...
            completeServico = $('#servicoBusca')
                .autocomplete({
                    ...
                    onSelect: function(value, data) {
                        ...
                        document.form
                            .fireEvent('carregaBaseConhecimentoAssoc');
...
$('html')
    .on(
        ...
        function(e) {
            if ($(e.target).is('.search') || !$(e.target).parents('li').is(
                    '.dropdownFiltroPesquisa')) {
...
function chamarModalDescricaoSolicitacao(descricao, idSolicitacaoServico) {
    $('#contentFrameDescricaoSolicitacao')
        .html(
            '<h3>' + i18n_message('solicitacaoServico.solicitacao') + '&nbsp;&nbsp;' + idSolicitacaoServico + '</h3><br><div class=\"box-generic\"><div class=\"descricaoModal\">' + descricao + '</div></div>');
    $('#modal_descricaoSolicitacao').modal('show');
}

geraPopoverInformacoesSolicitante = function(telefoneContato, emailContato) {

    if ((emailContato != '' && emailContato != undefined && emailContato != null) || (telefoneContato != '' && telefoneContato != undefined && telefoneContato != null)) {
        var strAux1 = '';
        ...
        if (emailContato != '' && emailContato != undefined && emailContato != null) {
            /*
             * Rodrigo Pecci Acorse - 20/11/2013 16h40 - #125019 Foi adicionada
             * a classe nowrap para evitar a quebra de linha do e-mail
             */
            strAux2 += "<span class='nowrap'>" + i18n_message('citcorpore.comum.email') + "</span>: " + emailContato;
        }

        $('.informacoesSolicitante').attr(
            'data-content',
            '<p>' + '<label>' + strAux1 + '</label>' + '<label>' + strAux2 + '</label>' + '</p>');
...
$(document)
    .ready(
        function() {
            completeSolicitante = $('#nomeSolicitante')
                .autocomplete({
                    ...
                    onSelect: function(value, data) {
                        ...
                        document.formInformacoesContato
                            .fireEvent("restauraSolicitante");
                    }
                });
        });

function montaParametrosAutocompleteSol(idContrato) {
    completeSolicitante.setOptions({
        params: {
            contrato: idContrato
        }
    });
...
function carregarModalDuplicarSolicitacao(idSolicitacao) {
    ...
    var idGrupoAtual = $('select[name="idGrupoAtual"]').find(':selected').attr(
        'value');
    if (idGrupoAtual == "" || idGrupoAtual == "undefined")
        idGrupoAtual = "-1";
    ...
    document.formInformacoesContato
        .fireEvent('carregarModalDuplicarSolicitacao');
...
exibirSubSolicitacoes = function(idSolicitacaoServico) {
    ...
    document.formIncidentesRelacionados
        .fireEvent("abrirListaDeSubSolicitacoes");
 ...
 * $('#conteudoiframeInformacaoItemConfiguracao').html('<iframe
 * src="about:blank" width="99%" id="iframeInformacaoItemConfiguracao"
 * height="530" class="iframeSemBorda"></iframe>'); }
 */
    ...
    completeUnidade = $('#unidadeDes')
        .autocomplete({
            ...
            onSelect: function(value, data) {
                ...
                document.formInformacoesContato
                    .fireEvent("preencherComboLocalidade");
            }
        });
}
...
/*
 * Function de validação para preenchimento de valores apenas numerico de acordo
 * com a iniciativa 492.
 *
 * @author Ezequiel Bispo Nunes @date 2014-12-05
 *
 */
function ajustarCampoNumeroSolicitacao() {

    jQuery("#idSolicitacao").keypress(
        function(el) {
            ...
            /*
             * A: 65 e 97 C: 99 e 67 V: 86 e 118 X: 88 e 120
             */
...
function gravarSolicitacao22() {
    ...
    if (document.form.descricao.value == '' || document.form.descricao.value == '&nbsp;' || document.form.descricao.value == '<p></p>') {
        alert('Informe a descrição!');
        return;
    }
    ...
    if (objsMudanca != null) {
        document.form.colItensMudanca_Serialize.value = ObjectUtils
            .serializeObjects(objsMudanca);
    }

    var objsIC = HTMLUtils.getObjectsByTableId('tblIC');
    if (objsIC != null) {
        document.form.colItensIC_Serialize.value = ObjectUtils
            .serializeObjects(objsIC);

        var objsBaseConhecimento = HTMLUtils
            .getObjectsByTableId('tblBaseConhecimento');
        document.form.colConhecimentoSolicitacao_Serialize.value = ObjectUtils
            .serializeObjects(objsBaseConhecimento);
    }
            ...
            if (parseFloat(solicitacaoDto.atrasoSLA) > parseFloat("0,00") && solicitacaoDto.situacao != situacaoSolicitacaoServico_Suspensa && solicitacaoDto.situacao != situacaoSolicitacaoServico_Cancelada) {
                ...
                if (qtdeItens < 15) {
                    if (solicitacaoDto.slaACombinar && solicitacaoDto.slaACombinar != 'S' && solicitacaoDto.situacaoSLA == 'A') {
                        scriptTemposSLA += "temporizador.addOuvinte(new Solicitacao('tempoRestante" + solicitacaoDto.idSolicitacaoServico + "', " + "'barraProgresso" + solicitacaoDto.idSolicitacaoServico + "', " + "'" + solicitacaoDto.dataHoraInicioSLAStr + "', '" + solicitacaoDto.dataHoraLimiteToString + "'));";
...
/*
 * Rodrigo Pecci Acorse 07/11/2013 - Sol. 123390 - Os iframs que possuiam src
 * definido aqui no load da página foram removidos e adicionados somente na ação
 * do item. Os frames serão carregados somente quando necessário. - A altura do
 * iframe foi removida pois estava causando 2 barras de rolagem na página de
 * nova solicitação.
 */
$(window)
    .load(
        function() {
            $('#contentFrameNovaSolicitacao')
                .html(
                    '<iframe src="about:blank" id="frameNovaSolicitacao" width="100%" class="iframeSemBorda"></iframe>');
            $('#contentFrameAlterarSLA')
                .html(
                    '<iframe id="frameAlterarSLA" src="about:blank" width="100%" height="520" class="iframeSemBorda"></iframe>');
            $('#contentReclassificarSolicitacao2')
                .html(
                    '<iframe id="frameReclassificarSolicitacao2" src="about:blank" width="100%" height="600" class="iframeSemBorda"></iframe>');
            $('#contentFrameAgendarAtividade')
                .html(
                    '<iframe id="frameAgendarAtividade" src="about:blank" width="100%" height="460" class="iframeSemBorda"></iframe>');
            $('#contentFrameagendaAtvPeriodicas')
                .html(
                    '<iframe id="frameAgendaAtvPeriodicas" src="about:blank" width="100%" height="530" class="iframeSemBorda"></iframe>');
            $('#contentFrameSuspenderReativarSolicitacao')
                .html(
                    '<iframe id="frameSuspenderReativarSolicitacao" src="about:blank" width="100%" height="520" class="iframeSemBorda"></iframe>');
            $('#contentFrameExibirDelegacaoTarefa')
                .html(
                    '<iframe id="frameExibirDelegacaoTarefa" src="about:blank" width="100%" height="400" class="iframeSemBorda"></iframe>');
            $('#contentFrameSuspender')
                .html(
                    '<iframe id="frameExibirSuspender" src="about:blank" width="100%" height="400" class="iframeSemBorda"></iframe>');
            ...
            $('#contentframeReclassificarSolicitacao')
                .html(
                    '<iframe id="frameReclassificarSolicitacao" src="about:blank" width="100%" height="700" class="iframeSemBorda"></iframe>');
            $('#conteudoframeExibirOrigemSub')
                .html(
                    '<iframe id="frameExibirOrigemSub" src="about:blank" width="100%" height="350" class="iframeSemBorda"></iframe>');
            $('#conteudoPesquisaItemConfiguracao')
                .html(
                    '<iframe id="framePesquisaItemConfiguracao" src="about:blank" width="99%" height="530" class="iframeSemBorda"></iframe>');
            $('#conteudoiframeInformacaoItemConfiguracao')
                .html(
                    '<iframe id="iframeInformacaoItemConfiguracao" src="about:blank" width="99%" height="530" class="iframeSemBorda"></iframe>');
        });