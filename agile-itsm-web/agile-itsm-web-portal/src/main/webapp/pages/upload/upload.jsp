...
<script>
	...
	function excluirImagemUpload(path){
		if (confirm(i18n_message("citcorpore.comum.confirme.desejaexcluiranexo"))){
			window.location = '${ctx}/pages/uploadExcluir/uploadExcluir.load?path=' + path;
		}
	}
	...
	<thead>
        ...
        if(flagGerenciamento!= null && flagGerenciamento.equalsIgnoreCase("S")){
      	...
      	<TR>
			...
			%>
			<td >
					<input type="image" id='btnExcluirAnexo' src='${ctx}/imagens/delete.png' style='cursor:pointer' onclick='excluirImagemUpload("<%=uploadDTO.getPath()%>")' title='<fmt:message key="citcorpore.ged.msg.exluiranexo" /> '/>
			</td>
			<td >
        ...
        <%}else if (idServicoUp.equals(uploadDTO.getIdLinhaPai())){ %>
